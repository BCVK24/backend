#протестируем только транскрибацию
from asr_inference import ASRInference
from utils import convert_wav, filter, normalize_transcribe
from main import model_config


def test_transcribe_model_GigaAM():
    
    """тест транскрибации модели GigaAM, на записи 60 секунд"""
    
    ETALON_OUTPUT = normalize_transcribe(['то', 'как', 'мы', 'его', 'воспринимаем', 'нравится', 'он', 'нам', 'или', 'нет', 'кажется', 'он', 'песклявым', 'или', 'наоборот', 'бархатным',
    ...
    for device in devices:
        ...
        output = normalize_transcribe(model.transcribe(speech))
        assert output == ETALON_OUTPUT
        
        
        
def test_transcribe_model_Whisper():
    
    """тест транскрибации модели Whisper, на записи 60 секунд"""
    ...
    for device in devices:
        model_config['device'] = device
        model = ASRInference('Whisper', model_config)
        output = normalize_transcribe(model.transcribe(speech))
        assert output == ETALON_OUTPUT


def test_transcribe_model_Vosk():
    ...
    for device in devices:
        model_config['device'] = device
        model = ASRInference('Vosk', model_config)
        output = normalize_transcribe(model.transcribe(speech))
        assert output == ETALON_OUTPUT